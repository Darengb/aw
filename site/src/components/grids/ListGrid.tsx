...
export function ListGrid({ items, columns = 3 }: ListGridProps) {
  return (
    <div
      ...
      {items.map((item, index) => {
        ...
        return (
          <div
            key={index}
            className={`
              group relative flex items-center gap-4 p-8 bg-transparent border-b border-[var(--gray-200)] transition-all duration-400
          ...
          >
            ...
            <p className="text-[var(--text-base)] text-[var(--gray-800)] leading-relaxed font-medium">
              {item.text}
            </p>
          </div>
        )
